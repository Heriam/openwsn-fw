...
enum {
   ...
   COMPONENT_BIER_TO_IEEE802154E	   = 0x0b,
   COMPONENT_IEEE802154E_TO_SIXTOP     = 0x0c,
   COMPONENT_IEEE802154E_TO_BIER	   = 0x0d,
   //MAChigh
   COMPONENT_SIXTOP                    = 0x0e,
   COMPONENT_NEIGHBORS                 = 0x0f,
   COMPONENT_SCHEDULE                  = 0x10,
   COMPONENT_SIXTOP_RES                = 0x11,
   COMPONENT_BIER					   = 0x12,
   //IPHC
   COMPONENT_OPENBRIDGE                = 0x13,
   COMPONENT_IPHC                      = 0x14,
   //IPv6
   COMPONENT_FORWARDING                = 0x15,
   COMPONENT_ICMPv6                    = 0x16,
   COMPONENT_ICMPv6ECHO                = 0x17,
   COMPONENT_ICMPv6ROUTER              = 0x18,
   COMPONENT_ICMPv6RPL                 = 0x19,
   //TRAN
   COMPONENT_OPENTCP                   = 0x1a,
   COMPONENT_OPENUDP                   = 0x1b,
   ...
   COMPONENT_TECHO                     = 0x24,
   COMPONENT_TOHLONE                   = 0x25,
...
enum {
   ...
   ERR_BIER_FORWARDED				   = 0x19, // tried to forward a bier message to next hop
   ERR_BIER_DIFFERENT_MSG			   = 0x1a, // Two different messages from the same track in the queue
   // l2a
   ERR_WRONG_CELLTYPE                  = 0x1b, // wrong celltype {0} at slotOffset {1}
   ERR_IEEE154_UNSUPPORTED             = 0x1c, // unsupported IEEE802.15.4 parameter {1} at location {0}
   ERR_DESYNCHRONIZED                  = 0x1d, // got desynchronized at slotOffset {0}
   ERR_SYNCHRONIZED                    = 0x1e, // synchronized at slotOffset {0}
   ERR_LARGE_TIMECORRECTION            = 0x1f, // large timeCorr.: {0} ticks (code loc. {1})
   ERR_WRONG_STATE_IN_ENDFRAME_SYNC    = 0x20, // wrong state {0} in end of frame+sync
   ERR_WRONG_STATE_IN_STARTSLOT        = 0x21, // wrong state {0} in startSlot, at slotOffset {1}
   ERR_WRONG_STATE_IN_TIMERFIRES       = 0x22, // wrong state {0} in timer fires, at slotOffset {1}
   ERR_WRONG_STATE_IN_NEWSLOT          = 0x23, // wrong state {0} in start of frame, at slotOffset {1}
   ERR_WRONG_STATE_IN_ENDOFFRAME       = 0x24, // wrong state {0} in end of frame, at slotOffset {1}
   ERR_MAXTXDATAPREPARE_OVERFLOW       = 0x25, // maxTxDataPrepare overflows while at state {0} in slotOffset {1}
   ERR_MAXRXACKPREPARE_OVERFLOWS       = 0x26, // maxRxAckPrepapare overflows while at state {0} in slotOffset {1}
   ERR_MAXRXDATAPREPARE_OVERFLOWS      = 0x27, // maxRxDataPrepapre overflows while at state {0} in slotOffset {1}
   ERR_MAXTXACKPREPARE_OVERFLOWS       = 0x28, // maxTxAckPrepapre overflows while at state {0} in slotOffset {1}
   ERR_WDDATADURATION_OVERFLOWS        = 0x29, // wdDataDuration overflows while at state {0} in slotOffset {1}
   ERR_WDRADIO_OVERFLOWS               = 0x2a, // wdRadio overflows while at state {0} in slotOffset {1}
   ERR_WDRADIOTX_OVERFLOWS             = 0x2b, // wdRadioTx overflows while at state {0} in slotOffset {1}
   ERR_WDACKDURATION_OVERFLOWS         = 0x2c, // wdAckDuration overflows while at state {0} in slotOffset {1}
   // general
   ERR_BUSY_SENDING                    = 0x2d, // busy sending
   ERR_UNEXPECTED_SENDDONE             = 0x2e, // sendDone for packet I didn't send
   ERR_NO_FREE_PACKET_BUFFER           = 0x2f, // no free packet buffer (code location {0})
   ERR_FREEING_UNUSED                  = 0x30, // freeing unused memory
   ERR_FREEING_ERROR                   = 0x31, // freeing memory unsupported memory
   ERR_UNSUPPORTED_COMMAND             = 0x32, // unsupported command {0}
   ERR_MSG_UNKNOWN_TYPE                = 0x33, // unknown message type {0}
   ERR_WRONG_ADDR_TYPE                 = 0x34, // wrong address type {0} (code location {1})
   ERR_BRIDGE_MISMATCH                 = 0x35, // bridge mismatch (code location {0})
   ERR_HEADER_TOO_LONG                 = 0x36, // header too long, length {1} (code location {0})
   ERR_INPUTBUFFER_LENGTH              = 0x37, // input length problem, length={0}
   ERR_BOOTED                          = 0x38, // booted
   ERR_INVALIDSERIALFRAME              = 0x39, // invalid serial frame
   ERR_INVALIDPACKETFROMRADIO          = 0x3a, // invalid packet frome radio, length {1} (code location {0})
   ERR_BUSY_RECEIVING                  = 0x3b, // busy receiving when stop of serial activity, buffer input length {1} (code location {0})
   ERR_WRONG_CRC_INPUT                 = 0x3c, // wrong CRC in input Buffer (input length {0})
   ERR_PACKET_SYNC                     = 0x3d, // synchronized when received a packet
   ...
   ERR_TEST_RCVD_MSG_2                 = 0x52, // received a message from track 2
